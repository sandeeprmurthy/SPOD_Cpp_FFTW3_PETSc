...
int main(int argc, char** argv) {
    ...
    std::string file_name = "./jetLES.h5";

    auto [data, grid, dt, ng, nt, nx, nvar, weight] = processHDF5File(file_name);

    // Output results
    rank_0_cout(rank, "Time step (dt): ",dt," seconds");
    rank_0_cout(rank, "Number of grid points (ng): ",ng);
    rank_0_cout(rank, "Number of snapshots (nt): ",nt);
    rank_0_cout(rank, "Number of variables (nvar): ",nvar);
    rank_0_cout(rank, "First 5 weights: ");
    for (int i = 0; i < std::min(5, static_cast<int>(weight.size())); ++i) {
        rank_0_cout(rank, weight[i], " ");
    }
    
    for (int i = 0; i < std::min(5, static_cast<int>(data.size())); ++i) {
        rank_0_cout(rank, data[i], " ");
    } 
    
    std::vector<double> mean_data(nx);
    for (int i = 0; i < nx; ++i) {
    		mean_data[i] = 0.0;
    		for (int t = 0; t < nt; ++t) {
        	mean_data[i] += data[t*nx + i]/nt; // Accessing the 3rd column
        }
    }       

	// ####################################################################################    

    std::vector<ptrdiff_t> N0s = {ng}; // Adjust these dimensions
    std::vector<ptrdiff_t> N1s = {1};
    std::vector<ptrdiff_t> N2s = {1};    
...
////        std::stringstream ss;
////        ss << "/scratch1/03119/srmurth2/new_box/Sandeep_PhD_Aeroacoustics/LES_PlasCom2_Simulation_Data/Full_Domain_Single_Jet_TTR_1_With_Downstream_Sponge/PlasCom2_" << t * DELTA_ITERATION + 373000 << ".h5";
...
//	time_step_files.push_back("./PlasCom2_000890400.h5");   
	...
	// Allocation of data_buffers with modifications
...
//				if(seg==0){                    
//					for (int i = 0; i < std::min(5, static_cast<int>(data.size())); ++i) {
//						rank_0_cout(rank, "data[0 * total_local_size + i] = ",data[0 * total_local_size + i]," mean_data[i] = ",mean_data[i]," (data[0 * total_local_size + i] - mean_data[i]) = ",(data[0 * total_local_size + i] - mean_data[i]));
//					}  
//					return;      
//				}     
...
// ####################################################################################	
	...
	Vec weight_vec_petsc;
	Mat W;

	// Convert std::vector to PETSc Vec
	CreatePetscVecFromStdVector(weight, weight_vec_petsc);

  // Create a sparse diagonal matrix from the weight vector
  MatCreate(PETSC_COMM_WORLD, &W);
  MatSetSizes(W, PETSC_DECIDE, PETSC_DECIDE, total_global_size * NUM_VARIABLES, total_global_size * NUM_VARIABLES);
  MatSetType(W, MATAIJ); // Specify sparse matrix type
  MatSetFromOptions(W);
  MatSetUp(W);
